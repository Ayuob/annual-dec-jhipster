...
describe('FamilyMember Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'family-member', component: FamilyMemberComponent }]), HttpClientTestingModule],
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.familyMembers?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to familyMemberService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getFamilyMemberIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getFamilyMemberIdentifier).toHaveBeenCalledWith(entity);
