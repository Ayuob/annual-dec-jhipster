...
type RestOf<T extends IDocument | NewDocument> = Omit<T, 'submissionDate'> & {
...
export class DocumentService {
  ...
  protected convertDateFromClient<T extends IDocument | NewDocument | PartialUpdateDocument>(document: T): RestOf<T> {
    return {
      ...document,
      submissionDate: document.submissionDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDocument: RestDocument): IDocument {
    return {
      ...restDocument,
      submissionDate: restDocument.submissionDate ? dayjs(restDocument.submissionDate) : undefined,
    };
  }
