...
describe('Document e2e test', () => {
  ...
  const documentSample = { fileName: 'Games Account SQL', filePath: 'reciprocal', submissionDate: '2023-08-23T08:03:31.307Z' };
  ...
  describe('new Document page', () => {
    ...
    it('should create an instance of Document', () => {
      cy.get(`[data-cy="fileName"]`).type('Books Handcrafted copying').should('have.value', 'Books Handcrafted copying');

      cy.get(`[data-cy="filePath"]`).type('ivory applications').should('have.value', 'ivory applications');

      cy.get(`[data-cy="submissionDate"]`).type('2023-08-23T05:47').blur().should('have.value', '2023-08-23T05:47');

      cy.get(`[data-cy="documentdType"]`).select('OTHER');
