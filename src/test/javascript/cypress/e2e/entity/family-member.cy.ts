...
describe('FamilyMember e2e test', () => {
  ...
  const familyMemberSample = {
    nationalNumber: 'Lead parseXX',
    name: 'reboot portals modular',
  ...
  describe('new FamilyMember page', () => {
    ...
    it('should create an instance of FamilyMember', () => {
      cy.get(`[data-cy="nationalNumber"]`).type('Quality-focu').should('have.value', 'Quality-focu');

      cy.get(`[data-cy="name"]`).type('FTP').should('have.value', 'FTP');

      cy.get(`[data-cy="dateOfBirth"]`).type('2023-08-23').blur().should('have.value', '2023-08-23');

      cy.get(`[data-cy="gender"]`).type('viral Practical').should('have.value', 'viral Practical');
